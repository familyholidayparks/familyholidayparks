...
function plainPinContent(name) {{
  ...
  ">
    <div style="font-size:10px;font-weight:600;color:#fff;">${{escapeHtml(name)}}</div>
  </div>`;
  return wrap;
}}
...
function refreshAllPins() {{
  ...
  Object.entries(markersBySlug).forEach(([slug, entry]) => {{
    entry.marker.content = activeSlug === slug
      ...
      : plainPinContent(entry.loc.name);
  }});
}}
...
function initMap() {{
  ...
  LOCATIONS.forEach(loc => {{
    ...
    const marker = new google.maps.marker.AdvancedMarkerElement({{
      ...
      content: plainPinContent(loc.name),
      title: loc.name,
    }});
