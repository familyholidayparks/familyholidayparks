...
@media (min-width: 768px) {{
  ...
  .locations-intro {{ padding: 24px 24px 16px; }}
...
<div class="locations-section">
  <div class="locations-intro">
    <h2>Browse Australia's Best Holiday Park Destinations</h2>
    <p>Ranked from 300,000+ real family reviews.</p>
    <span class="locations-count">{len(card_locations)} holiday park destinations</span>
  </div>
  {location_cards_html}
</div>
...
function refreshAllPins() {{
  const activeSlug = hoverLabelSlug || scrollLabelSlug;
  Object.entries(markersBySlug).forEach(([slug, entry]) => {{
    entry.marker.content = activeSlug === slug
...
function initMap() {{
  ...
  document.querySelectorAll('.lcard').forEach(card => {{
    card.addEventListener('mouseenter', () => {{
      hoverLabelSlug = card.dataset.slug;
      refreshAllPins();
    }});
